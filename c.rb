...
#{make_null_check("out_value", "S2_VALUE_IS_NULL")}
	if (in_index >= in_object->COUNT_#{field_name})
	{
		return S2_INDEX_OUT_OF_BOUNDS;
	}

	*out_value = in_object->BUFFER_#{field_name}[in_index];
	return S2_NO_ERROR;
}
			GET_DEFINITION
...
#{make_null_check("in_object", "S2_OBJECT_IS_NULL")}
	...
	in_object->BUFFER_#{field_name}[in_index] = in_value;
	return S2_NO_ERROR;
}
			SET_DEFINITION
...
{
#{make_null_check("in_object", "S2_OBJECT_IS_NULL")}
	*out_value = in_object->COUNT_#{field_name};
	return S2_NO_ERROR;
}
			COUNT_DEFINITION
			...
			function_defines += <<-PROTOTYPES
S2Error Count#{structure_name}_#{field_name}(#{structure_name}* in_object, #{field_type}* out_value)#{count_definition}
S2Error Get#{structure_name}_#{field_name}(#{structure_name}* in_object, size_t in_index, #{field_type}* out_value)#{get_definition}
S2Error Set#{structure_name}_#{field_name}(#{structure_name}* in_object, size_t in_index, #{field_type} in_value)#{set_definition}
			PROTOTYPES
		else
